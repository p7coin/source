...
// Copyright (c) 2011-2012 P7Coin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #P7CoinTEST0\r");
            Send(hSocket, "WHO #P7CoinTEST0\r");
        } else {
            // randomly join #P7Coin00-#P7Coin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #P7Coin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #P7Coin%02d\r", channel_number).c_str());
        }
