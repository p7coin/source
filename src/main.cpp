...
uint256 hashGenesisBlock("0x0b63120a30a223474a8bf729665e28069d4b7c380a7432a70b32efcf9d021c14");
...
const string strMessageMagic = "P7Coin Signed Message:\n";
...
{
    // P7Coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 0.5 * 60;
...
{
    // P7Coin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1428695919;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 752664838;
	    ...
	    assert(block.hashMerkleRoot == uint256("0xe1d4c1b259adfb9e0117a17e61d68c71ef587571101223197b2c9c05f5e4dbd9"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("P7CoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("P7CoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static P7CoinMiner(CWallet *pwallet)
{
    printf("P7CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("P7Coin-miner");
    ...
    try { loop {
        ...
        printf("Running P7CoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&P7CoinMiner, pwallet));
}
